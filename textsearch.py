...
def getResults(wordinput):                                                                                                          
    ...
    for fName in txtFilenames:
        ...
        if isMatch:
            ...
            tempResult.type = fName.split('-')[0].split('_')[1]
            parts = fName.split('-')[1:]
            parts[-1] = parts[-1].split('.')[0]  # remove txt 
            year = parts[-1].split('_')[1]   # get out year 
            parts[-1] = parts[-1].split('_')[0]  # city name 
            name = ""
            for part in parts:
                name += part + " "   # add all components of city name 
            name = name[:-1]  # removing last space 
            tempResult.cityName = name   # assigning city name to name 
            tempResult.year = year   #
            tempResult.occurences = occurences
            if tempResult.type == 'county':
                tempResult.population = int(countyPops[tempResult.cityName])
