...
int main(void){
	...
	fileListFile.open("FILELIST_SHORT.txt"); //FILELIST.txt contains a list of all of the cities FILELIST_SHORT.txt contains only a 3 
	    ...
	    if (cityPlanFile.is_open()){   //checking whether the file is open
		    ...
		    while(getline(cityPlanFile, line)){ //read data from file object and put it into string.
		    	...
		    	do{
		         	string word;
		         	ss >> word; 
		         	bool contains_non_alpha = 0; //!std::regex_match(word, std::regex("^[A-Za-z]+$")); //removes bad keys from hash map
	...
	write_wordMap_to_file("test.txt",&wordMap);
